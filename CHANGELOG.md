...
- **Change Tracking (chg) Display**: Dictionary entries with `<chg>` tags now render the old value with red strikethrough and the new value in green inline. The full change metadata (date, user, note, reference) is available via a tappable SnackBar tooltip. Transliteration respects `<s>`/`<s1>` wrappers and Sanskrit context.
- **MW Literary Source Coverage**: Fixed MW pattern routing in `ls_patterns.dart` — MW now uses its own dedicated 150+ pattern set instead of incorrectly falling back to AP patterns. MW LS coverage improved from ~51% to 100% (42,819/42,819).
- **AP Literary Sources**: Literary source (LS) links now correctly generated for the AP dictionary.
- **AP90 Literary Source Tests**: Added dedicated test suite for AP90 dictionary LS URL generation.

### Fixed

- **MW LS Pattern Matching**: 8 MW-specific LS patterns (BhP., Bhag., Ragh., Megh., Pañcat., Mn., R. Bombay) no longer return `null` or wrong URLs. Fixes regression in LS service unit tests and full MW coverage tests.

### Changed

- **Runbook Documentation**: Added tooling runbook files — CLAUDE.md, README.md, CITATION.cff, CONTRIBUTING.md, CODE_OF_CONDUCT.md, and live coverage stats.
